import streamlit as st    # I used streamlit here it let us create web pages with pure python
import pandas as pd       # no html, css or javascript needed
import numpy as np         # streamlit for web frame work, pandas and numpy for data handling
import joblib              # joblib to load our saved model
import plotly.graph_objects as go # plotly for beautifull and interactive chart
...
@st.cache_resource                # it caches the model without this streamlit would reload the model every time u interact with the app.
def load_model_and_scaler():                    # here we r loading a saved model.
    try:
        model = joblib.load('diabetes_model.pkl')      # if files are found return model,scaler otherwise return none.
...
# Header
st.title("🏥 Diabetes Prediction System")   # it creates a big header

# Load model
model, scaler = load_model_and_scaler()

if model is None or scaler is None:
    st.error("❌ **Model files not found!**")    # now checking model exists or not if not show an error and stop
    st.info("""                                   
...
# Sidebar inputs
st.sidebar.title("⚙️ Patient Information")    # sts sidebar puts everything in the sidebar.
st.sidebar.subheader("Demographics")           
age = st.sidebar.slider('Age', 21, 100, 30)    # 21 maximum 100 and default is 30
pregnancies = st.sidebar.number_input('Pregnancies', 0, 20, 0)  # number input creates a number box u can type or use arrows
